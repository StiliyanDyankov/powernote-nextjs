...
import { webpage2 } from "@/utils/webpage2";
    ...
    return (
        <div className=" w-full h-full p-2">
            <div className=" h-full border border-l-divider/70 px-1 py-2 bg-primary/50 dark:bg-d-400-sibebar rounded-md overflow-hidden">
                <div className=" pb-2 pl-3 font-medium dark:text-l-workscreen-bg/70">Clipped from the web <span className=" italic"> - from <a target="_blank" href={currentNote?.from} className=" hover:underline cursor-pointer"> {currentNote?.noteName}</a></span></div>
                    <iframe srcDoc={ currentNote?.noteName === "react-quill - npm" ? webpage : webpage2}  className="w-full h-full rounded-md">
                    </iframe>
